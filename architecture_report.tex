...
\begin{excalidrawbox}
\textbf{\large SIMPLIFIED ARCHITECTURE SPECIFICATION: 4-Phase KernelLens Compiler Pipeline} \\
\textit{Use this simplified 4-phase flow layout for Figure 1 in Excalidraw:}

\smallskip
\textbf{1. Phase 1: Dual-Pass Tracing \& AST Inspection (Analysis)} \\
PyTorch Model $\xrightarrow{\text{Pass 1 Eager}}$ Intercept PTX Bytecode \& $S_{\text{mem}}$ $\xrightarrow{\text{Pass 2 FX Trace}}$ SymPy Grid Expressions $\xrightarrow{\text{AST Inspector}}$ Classify Read/Write/In-Place Mutability $\rightarrow$ \texttt{KernelManifest}.

\smallskip
\textbf{2. Phase 2: Base ONNX Graph Transformation (Graph Modification)} \\
\texttt{torch.onnx.export} $\xrightarrow{\text{Exporter Hook}}$ Intercept Triton Calls $\rightarrow$ Inject \texttt{triton\_custom::<kernel>} ONNX Node $\rightarrow$ Bind Host CPU Scalars (\texttt{OrtMemTypeCPUInput}) $\rightarrow$ Alias In-Place Output Buffers.

\smallskip
\textbf{3. Phase 3: Automated C++/CUDA Code Synthesis \& Hardening (Code Compilation)} \\
Manifest $\xrightarrow{\texttt{ort\_gen/trt\_gen}}$ Synthesize \texttt{OrtCustomOp} \& \texttt{IPluginV2DynamicExt} C++ Plugins $\xrightarrow{\text{Embed Guards}}$ 16-Byte Alignment Guards $\xrightarrow{\text{Toolchain Build}}$ Blackwell PTX 9.0 Clamping + \texttt{patchelf} Stack Clearance $\rightarrow$ \texttt{libtriton\_plugins.so}.

\smallskip
\textbf{4. Phase 4: Zero-Copy Runtime Execution \& Pointer Binding (Runtime Execution)} \\
Dynamic \texttt{ctypes.CDLL} Load $\xrightarrow{\text{IOBinding}}$ Map VRAM Memory Addresses Directly to Engine Contexts $\rightarrow$ Zero-Copy Execution ($\text{MaxDiff} = 0.00\text{e}+00$).
\end{excalidrawbox}
...
\smallskip
$\bullet$ \textbf{Type:} Grouped Bar Chart (Log Scale Latency in ms, Lower is Better) \\
$\bullet$ \textbf{X-Axis (Operators):} Fused RMSNorm, SwiGLU Activation, RoPE Embedding, Fused Cross-Entropy \\
$\bullet$ \textbf{Bar Series (PyTorch Eager):} 24.12 ms (RMSNorm), 0.05 ms (SwiGLU), 8.01 ms (RoPE), 0.86 ms (CrossEntropy) \\
$\bullet$ \textbf{Bar Series (torch.compile):} 6.97 ms (RMSNorm), 0.20 ms (SwiGLU), 1.77 ms (RoPE), 0.22 ms (CrossEntropy) \\
$\bullet$ \textbf{Bar Series (Native Triton):} 7.03 ms (RMSNorm), 0.11 ms (SwiGLU), 1.90 ms (RoPE), 0.23 ms (CrossEntropy) \\
$\bullet$ \textbf{Bar Series (KernelLens ORT):} 7.19 ms (RMSNorm), 0.42 ms (SwiGLU), 2.04 ms (RoPE), 0.39 ms (CrossEntropy) \\
$\bullet$ \textbf{Annotation Callout:} \textit{"KernelLens matches native Triton execution latency while running inside pure C++ ONNX Runtime without Python dependencies!"}
\end{excalidrawbox}
